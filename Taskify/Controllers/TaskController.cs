    ...
    public class TaskController : ControllerBase
    {
        ...
        public async Task<ActionResult<IEnumerable<TasksDTO>>> GetTasksByProject(int projectId)
        {
            ...
            foreach (var task in tasks)
            {
                ...
                var worker = await _userService.GetUserByIDAsync(task.UserId);
                dto.UserName = worker?.Name;
                tasksDto.Add(dto);
            }
            return Ok(tasksDto);
        }

        // GET api/<TaskController>/manager/5
        [HttpGet("manager/{managerId}")]
        [Authorize(Roles = "manager,headmanager")]
        public async Task<ActionResult<IEnumerable<TasksDTO>>> GetTasksByManager(int managerId)
        {
            var email = User.FindFirst(ClaimTypes.Email)?.Value;
            var role = User.FindFirst(ClaimTypes.Role)?.Value;
            var currentUser = await _userService.GetUsersByEmailAndRoleAsync(email, role);
            ...
            // A regular manager can only access their own scope.
            ...
            if (currentUser.Id == userid)
            {
                ...
                foreach (var task in tasks)
                {
                    ...
                    if (project != null)
                    {
                        ...
                        // Prefer navigation (loaded with project); fallback to explicit user lookup.
                        var manager = project.Manager;
                        if (manager == null && project.ManagerId > 0)
                        {
                            manager = await _userService.GetUserByIDAsync(project.ManagerId);
                        }
                        taskDto.ProjectManagerName = manager?.Name;
                        taskDto.ProjectManagerEmail = manager?.Email;
                    }
